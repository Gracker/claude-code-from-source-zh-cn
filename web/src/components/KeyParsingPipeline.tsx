...
const stages: PipelineStage[] = [
  {
    ...
    shortLabel: "stdin",
    description:
      "原始位元组从终端到达。单次 read() 可能包含完整的跳脱序列或仅是片段。分词器以 50ms 逾时缓冲不完整的序列。",
  },
  {
    id: "protocol",
    label: "协议侦测",
    shortLabel: "协议",
    description:
      "分类位元组序列：Kitty 键盘协议（CSI u）、xterm modifyOtherKeys（CSI 27;mod;key~）、传统 VT220、SGR 滑鼠事件，或括号贴上。",
  },
  {
    id: "parse",
    label: "序列解析",
    shortLabel: "解析",
    description:
      "将跳脱序列解码为按键识别 + 修饰键。XTerm 惯例：modifier = 1 + (shift?1:0) + (alt?2:0) + (ctrl?4:0) + (super?8:0)。",
  },
  {
    id: "event",
    label: "按键事件建立",
    shortLabel: "事件",
    description:
      "结构化 ParsedKey 对象：{ kind: 'key', name, ctrl, meta, shift, option, super, sequence, isPasted }。所有歧义已消除。",
  },
  {
    id: "binding",
    label: "键绑定查找",
    shortLabel: "绑定",
    description:
      "在所有 16 个活跃上下文的合并绑定表中比对。最后匹配的绑定产生效果（使用者覆写优先）。上下文清单在每次按键时重建。",
  },
  {
    id: "chord",
    label: "和弦处理",
    shortLabel: "和弦",
    description:
      "若是和弦的前半部分：等待最多 1000ms 接收后半部分。ChordInterceptor 在等待期间捕获所有输入。取消的和弦会丢弃前缀但让不匹配的字符通过。",
  },
  {
    id: "action",
    label: "动作分派",
    shortLabel: "动作",
    description:
      "执行绑定的动作处理器。stopImmediatePropagation() 阻止进一步处理。React 会批次处理所有结果状态更新。",
  },
];
...
const presets: KeyPreset[] = [
  ...
  },
  {
    ...
    actionDescription: "向上卷动一页",
  },
  {
    name: "Escape",
    rawBytes: "\\x1b",
    hexBytes: ["1b"],
    protocol: "模棱两可（50ms 逾时以区分 CSI 前缀）",
          ...
          {selectedPreset && activeStageIndex >= 6 && (
            ...
            >
              ...
              >
                ...
                <div>
                  <span style={{ color: colors.text }}>十六进位：</span>{" "}
                ...
                <div>
                  <span style={{ color: colors.text }}>解析结果：</span>{" "}
