...
`kind` 判别式确保下游代码明确处理每种输入类型。一个按键不可能被意外地当作滑鼠事件处理；一个终端响应不可能被意外地解释为按键。`ParsedKey` 类型还携带原始 `sequence` 字符串用于除错 —— 当用户回报「按 Ctrl+Shift+A 没有反应」时，除错日志能显示终端究竟发送了什么字节序列，使得诊断问题究竟出在终端的编码、解析器的识别还是键绑定的设置成为可能。
