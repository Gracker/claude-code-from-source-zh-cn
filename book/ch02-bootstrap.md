...
flowchart TD
    ...
    Init --> Setup["setup.ts<br/>命令、代理、钩子、外挂"]
...
每个文件只做最少的必要工作，然后将控制权交给下一个。`cli.tsx` 试图在引入任何重量级模块之前就退出。`main.tsx` 在模块求值期间以副作用的方式触发慢速操作。`init.ts` 解析配置并建立信任边界。`setup.ts` 注册各项能力。`replLauncher.ts` 选择正确的进入点并启动 UI。

三种并行策略使其快速：

1. **模块级子进程分发。** 在*模块求值期间*以副作用的方式触发钥匙串和 MDM 读取。这些子进程在剩余约 135 毫秒的静态引入加载期间同步执行。
2. **设置阶段的 Promise 并行化。** Socket 绑定、钩子快照、命令加载和代理定义加载全都并行执行。
3. **渲染后延迟预取。** 用户在输入第一条消息之前不需要的所有东西 —— git 状态、模型能力、AWS 凭据 —— 都在提示符可见之后才执行。

第四种策略较不明显但同样重要：**动态引入以延迟模块求值**。代码库在至少十几个地方使用 `await import('./module.js')` 来避免在需要之前加载代码。OpenTelemetry（400KB + 700KB gRPC）只在遥测初始化时加载。React 组件只在渲染时加载。每个动态引入以冷路径延迟（首次使用时触发模块求值）换取热路径速度（启动时不需要为可能永远用不到的模块付出代价）。
...
模式是：检查 `argv`，动态引入你需要的处理器，然后在系统其余部分加载之前退出。
...
大约有十几条快速路径涵盖版本、帮助、配置、MCP 服务器管理和更新检查。具体细节不重要 —— 重要的是模式。每条路径动态引入恰好一个模块，调用一个函数，然后退出。代码库的其余部分永远不会加载。

这是启动引导中反复出现的一个原则的首次体现：**通过更了解意图来减少工作**。argv 数组揭示了用户的意图。如果意图是狭窄的，执行路径也应该是狭窄的。

如果没有匹配任何快速路径，`cli.tsx` 会直接进入完整的 `main.tsx` 引入，真正的启动开始。

---

## 阶段 1：模块级 I/O（main.tsx）

当 `main.tsx` 被引入时，它的模块级副作用在求值期间触发 —— 在文件中的任何函数被调用之前。这是整个启动引导中最关键的性能技巧：
...
当 JavaScript 引擎求值 `main.tsx` 及其传递性引入的其余部分（约 138 毫秒的模块求值）时，这两个 Promise 已经在执行中了。MDM（行动设备管理）子进程检查组织的安全策略。钥匙串读取提取已存储的凭据。两者都是 I/O 密集型操作，否则会在关键路径上序列化。
...
`init()` 函数是记忆化（memoized）的 —— 多次调用是安全的，且返回相同的结果。这很重要，因为多个进入点（REPL、打印模式、SDK 模式）可能各自调用 `init()`，而记忆化保证它只执行一次。

此函数通过 Commander 解析命令列参数，从多个来源（全域设置、项目设置、环境变量）加载配置，然后到达管道中最重要的边界。
...
系统有十个不同的信任敏感操作。在用户接受信任对话框之前，只有安全操作会执行：TLS 凭据配置、主题偏好、遥测退出。信任之后，系统读取可能危险的环境变量（PATH、LD_PRELOAD、NODE_OPTIONS），执行 git 命令，并套用完整的环境配置。
...
命令、代理、钩子和外挂在可能的范围内全部并行注册。设置阶段是系统从「我知道我的配置」转变为「我拥有所有能力」的地方。设置完成后，每个工具都已注册，每个钩子都已连接，系统准备好处理用户输入。
...
七条不同的代码路径汇聚在 `replLauncher.ts`：交互式 REPL、打印模式（`--print`）、SDK 模式、恢复（`--resume`）、继续（`--continue`）、管道模式和无头模式。启动器检查 `init()` 产生的配置，并分发到正确的进入点。
...
| 模块求值 | 约 138ms | 引入树，触发并行 I/O |
| Commander 解析 | 约 3ms | 解析标志和子命令 |
| init() | 约 14ms | 配置解析、信任边界 |
| setup() | 约 35ms | 命令、代理、钩子、外挂 |
...
简要说明一个在 init 期间执行的子系统：结构描述迁移。Claude Code 将配置和会话数据存储在本地文件和目录中。当格式在版本之间变更时，迁移会在启动时自动执行。

每个迁移是一个带有版本号的函数。系统检查当前结构描述版本与最高迁移版本的对比，依次执行待处理的迁移，然后更新版本。迁移是幂等且快速的（操作的是小型本地文件，而非数据库）。整个迁移过程通常在 5 毫秒内完成。如果迁移失败，它会记录错误并继续 —— 对于本地配置来说，可用性胜过严格一致性。
...
**记忆化你的 init 函数。** 使初始化具有幂等性 —— 调用两次产生相同的结果。这消除了当多个进入点可能各自触发初始化时的排序错误。记忆化模式很简单，但消除了整类重复初始化的错误。
